...
#'
#' @usage \code{queue(n, arrive, use.full, wait.max = NULL, close.arrive = Inf, close.service = Inf, close.full = Inf)}
...
#' @param revival Revival-time for service facilities
...
print.queue <- function(OBJECT) {
  ...
  MATRIX   <- OBJECT$users.facilities
  n        <- OBJECT$n
  r        <- OBJECT$revival
...
plot.queue <- function(OBJECT, print = TRUE, gap = NULL, line.width = 2, 
...
print.summary.queue <- function(OBJECT) {
  
  #Check input
  if (!('summary.queue' %in% class(OBJECT))) stop('Error: Object must have class \'summary.queue\'' )
...
plot.summary.queue <- function(OBJECT, print = TRUE, count = FALSE,
