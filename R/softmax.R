...
#' and statistical applications.  The present functions define the softmax function and its inverse.
#'
#' @usage \code{softmax}
...
softmaxinv <- function(p, gradient = FALSE, hessian = FALSE) {

  #Compute the inverse-softmax function
  m <- length(p)
  if (m > 1) {
    SOFTINV <- (log(p) - log(p[m]))[1:(m-1)] } else {
    SOFTINV <- numeric(0) }
